  ...
  const fetchSummaryCounts = async () => {
    try {
      ...
      const [schemaRes, queriesRes] = await Promise.all([
        makeApiRequest('/fetch-count/schema', {
          method: 'POST',
          body: JSON.stringify({ market })
        }),
        makeApiRequest('/fetch-count/queries', {
          method: 'POST',
          body: JSON.stringify({ market })
        })
      ]);
      ...
      setSchemaAnalyzed(schemaRes.data?.[0]?.total_schemas || 0);
      setQueriesAnalyzed(queriesRes.data?.[0]?.total_schemas || 0);
    } catch (err) {
...
function formatLastUsed(timestamp) {
  ...
  const fetchRuleDetails = async (ruleId, pageNum) => {
    ...
    setRuleData(null); // Clear previous data
    console.log("fetchRuleDetails ", ruleId);
    try {
      const market = sessionStorage.getItem('selectedMarket') || '';
      const response = await makeApiRequest(`/rule`, {
        method: 'POST',
        body: JSON.stringify({
          rule_id: String(ruleId),  // Use dynamic ruleId
          market: market,
          page: pageNum + 1,
          page_size: ruleDataRowsPerPage
        }),
      });
      console.log('Rule Data API Response:', response); // Debug log

      setRuleData(Array.isArray(response.data) ? response.data : (response.data ? [response.data] : [])); // Ensure it's always an array
      setTotalRuleDataCount(response.total_count || (Array.isArray(response.data) ? response.data.length : (response.data ? 1 : 0))); // Calculate total count based on array or single object
                  ...
                  {displayRows.map((rec, index) => (
                    ...
                    >
                      ...
                      <TableCell align="center">
                        <Button
                          variant="text"
                          onClick={() => window.open(`/query-details/${rec.ruleId}/${encodeURIComponent(rec.recommendation)}`, '_blank')}
