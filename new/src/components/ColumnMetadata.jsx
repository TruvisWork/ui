...
const ColumnMetadata = () => {
  ...
  useEffect(() => {
    const fetchAuditHistory = async () => {
      if (!selectedMarket || !selectedTable || !selectedColumn) {
        setAuditHistory([]);
        return;
      }
    ...
    fetchAuditHistory();
  }, [selectedMarket, selectedTable, selectedColumn]);
    ...
    const fetchTableOptions = async () => {
      ...
      try {
        const response = await makeApiRequest('/get-tables', {
          method: 'POST', body: JSON.stringify({ market: selectedMarket })
        });
    ...
    const fetchColumnOptions = async () => {
      ...
      try {
        const response = await makeApiRequest('/get-columns', {
          method: 'POST', body: JSON.stringify({ market: selectedMarket, table_name: selectedTable })
        });
        if (!response || !response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        const data = await response.json();
  ...
  useEffect(() => {
    const fetchColumnMetadata = async () => {
      ...
      setError(null);
      try {
        const response = await makeApiRequest('/get-column-metadata', {
          method: 'POST', body: JSON.stringify({ market: selectedMarket, table_name: selectedTable, column_name: selectedColumn })
        });
        if (!response) return; // Auth error handled in makeApiRequest
        if (!response.ok) {
    ...
    try {
      ...
      if (selectedMarket && selectedTable && selectedColumn) {
          const auditResponse = await makeApiRequest('/get-audit-table', {
              method: 'POST',
              body: JSON.stringify({ market: selectedMarket, table_name: selectedTable, column_name: selectedColumn }),
          });
