...
public class SelectVisitor extends BaseStatementVisitor {
    ...
    public SqlNodeVisitor.Result visit(SqlCall call) {
        ...
        if (select.getWhere() != null) {
            ...
            result.getColumnInfos().addAll(ColumnExtractor.extractColumns(whereExpr, ClauseType.WHERE, dialect.product.name));
        ...
        if (select.getGroup() != null) {
            result.getColumnInfos().addAll(
                ColumnExtractor.extractGroupByColumns(select.getGroup(), select.getSelectList(), dialect.product.name)
            );
        }
    ...
    private void processJoin(SqlJoin join, SqlNodeVisitor.Result result) {
        ...
        if (join.getCondition() != null) {
            ...
            result.getColumnInfos().addAll(ColumnExtractor.extractColumns(join.getCondition(), ClauseType.JOIN, dialect.product.name));
