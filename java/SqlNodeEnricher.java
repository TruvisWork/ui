...
public class SqlNodeEnricher extends SqlShuttle {
    ...
    public SqlNode visit(SqlIdentifier id) {
        if (id instanceof SqlTableIdentifier tableId) {
            ...
            // Don't convert wildcards
            return id;
        } else {
            // Convert regular SqlIdentifier to SqlColumnIdentifier for column references
            // This ensures columns in SELECT lists and elsewhere get proper enrichment
            SqlColumnIdentifier columnId = new SqlColumnIdentifier(id.names, id.getParserPosition(), null, null);
            return columnQualifier.qualifyColumnIdentifier(columnId);
        }
    }
    ...
    private SqlNode visitFrom(SqlNode from) {
        ...
        if (from instanceof SqlJoin join) {
            ...
            // Handle table/view/CTE references with potential aliases
            SqlNode processedTable = accept(tableId);
            if (processedTable instanceof SqlTableIdentifier processedTableId) {
                String tableName = processedTableId.getTableName();

                // Check if this is a CTE reference
                if (tableName != null && scopeManager.hasSubquery(tableName)) {
                    // Don't add CTE as a table, but it's implicitly aliased to its own name
                    // The CTE is already registered in scopeManager from visitWith
                    log.debug("FROM references CTE '{}', skipping addTable", tableName);
                } else {
    ...
    private void trackTableFromNode(SqlNode node) {
        if (node instanceof SqlTableIdentifier tableId) {
            ...
            if ((tableNode instanceof SqlTableIdentifier || tableNode instanceof SqlViewIdentifier) &&
                ...
                String alias;
                SqlIdentifier aliasId = (SqlIdentifier) aliasNode;
                if (aliasId.names.size() == 1) {
                    alias = aliasId.getSimple();
                } else {
                    alias = aliasId.names.get(aliasId.names.size() - 1);
        ...
        for (SqlNode item : subquery.getSelectList()) {
            if (item instanceof SqlIdentifier id) {
                ...
                if (id instanceof SqlColumnIdentifier columnId) {
                    ...
                    log.debug("CTE projection - SqlColumnIdentifier: {} with entity={}, databaseEntity={}",
                              columnName,
                              sourceColumn != null ? sourceColumn.getName().getNormalizedName() : "null",
                              sourceEntity != null ? sourceEntity.getName().getNormalizedName() : "null");

                    if (sourceColumn == null || sourceEntity == null) {
                        log.warn("CTE projection - SqlColumnIdentifier {} missing entities! Attempting fallback resolution...", columnName);
                        // Fallback: try to resolve from the identifier's names
                        if (columnId.names.size() > 1) {
                            try {
                                List<String> tableNames = new ArrayList<>(columnId.names.subList(0, columnId.names.size() - 1));
                                EntityQualifier qualifier = new EntityQualifier(tableNames,
                                    List.of(defaultQualifiers.getDatabase(), defaultQualifiers.getSchema()),
                                    dialect);
                                sourceEntity = entityResolver.resolve(qualifier);
                                if (sourceEntity instanceof Table || sourceEntity instanceof View) {
                                    sourceColumn = createColumnEntity(sourceEntity, columnName);
                                    log.debug("CTE projection - Fallback resolution successful for: {}", columnName);
                                }
                            } catch (Exception e) {
                                log.debug("Fallback resolution failed for {}: {}", columnName, e.getMessage());
                            }
                        }
                    }
                } else {
                    ...
                    if (id.names.size() == 1) {
                        ...
                        // For multi-part identifiers, try to extract entity information
                        // e.g., database.schema.table.column or table.column
                        try {
                            List<String> tableNames = new ArrayList<>(id.names.subList(0, id.names.size() - 1));
                            EntityQualifier qualifier = new EntityQualifier(tableNames,
                                List.of(defaultQualifiers.getDatabase(), defaultQualifiers.getSchema()),
                                dialect);
                            sourceEntity = entityResolver.resolve(qualifier);
                            if (sourceEntity instanceof Table || sourceEntity instanceof View) {
                                sourceColumn = createColumnEntity(sourceEntity, columnName);
                                log.debug("CTE projection - Resolved entity from multi-part identifier: {} -> {}",
                                    id.names, sourceEntity.getName().getNormalizedName());
                            }
                        } catch (Exception e) {
                            log.debug("Failed to resolve entity for multi-part identifier: {}", id.names);
                        }
                    }
                }
                ...
                if (call.getOperandList().size() > 1 && call.getOperandList().get(1) instanceof SqlIdentifier aliasId) {
                    ...
                    // Extract source column from the expression
                    SqlNode expression = call.getOperandList().get(0);
                    Column sourceColumn = null;
                    DatabaseEntity sourceEntity = null;

                    // Try to extract source column - handle both direct columns and expressions (SUM, AVG, etc.)
