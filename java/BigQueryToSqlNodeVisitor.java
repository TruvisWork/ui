...
import com.calcite_new.sql.parser.bigquery.operators.NotEqualsOverrideOperator;
...
public class BigQueryToSqlNodeVisitor extends BigQuerySqlBaseVisitor<SqlNode> {
    ...
    @Override
    public SqlNode visitCreateTableStatement(BigQuerySqlParser.CreateTableStatementContext ctx) {
        SqlIdentifier tableName = (SqlIdentifier) visit(ctx.tableIdentifier(0));
        SqlNode query;

        boolean replace = ctx.OR() != null && ctx.REPLACE() != null;
        boolean ifNotExists = ctx.IF() != null && ctx.NOT() != null && ctx.EXISTS() != null;

        if (ctx.queryExpression() != null) {
            query = visit(ctx.queryExpression());
        ...
        throw new UnsupportedOperationException("Only CREATE TABLE AS SELECT is supported.");
    }

    @Override
    public SqlNode visitCreateViewStatement(BigQuerySqlParser.CreateViewStatementContext ctx) {
        SqlIdentifier viewName = (SqlIdentifier) visit(ctx.tableIdentifier());
        SqlNode query;
        ...
        if (ctx.columnName() != null && !ctx.columnName().isEmpty()) {
            List<SqlNode> columns = ctx.columnName().stream()
                    .map(this::visit)
                    .collect(Collectors.toList());
            columnList = new SqlNodeList(columns, parserPos(ctx));
        }
        ...
        throw new UnsupportedOperationException("Only CREATE VIEW AS SELECT is supported.");
    }

    @Override
    public SqlNode visitColumnName(BigQuerySqlParser.ColumnNameContext ctx) {
        // Explicitly create a column identifier for column names
        return new SqlColumnIdentifier(java.util.List.of(ctx.identifier().getText()), parserPos(ctx));
    }

    private SqlNode currentMergeTargetTable;

    @Override
    public SqlNode visitMergeStatement(BigQuerySqlParser.MergeStatementContext ctx) {
        SqlNode targetTable = visit(ctx.tableIdentifier());
        SqlNode source = visit(ctx.tableExpression());
        SqlNode onCondition = visit(ctx.expression());

        SqlIdentifier targetAlias = null;
        if (ctx.targetAlias != null) {
            targetAlias = new SqlIdentifier(ctx.targetAlias.getText(), parserPos(ctx.targetAlias));
            targetTable = SqlStdOperatorTable.AS.createCall(
                    parserPos(ctx),
                    targetTable,
                    targetAlias
            );
        ...
        this.currentMergeTargetTable = targetTable;

        List<SqlMerge.MatchedClause> matchedClauses = new ArrayList<>();
        SqlInsert insertCall = null;
        SqlNode insertCondition = null;
        for (BigQuerySqlParser.MergeWhenClauseContext whenCtx : ctx.mergeWhenClause()) {
            // Handle WHEN MATCHED clauses
            if (whenCtx.MATCHED() != null && whenCtx.mergeMatchedAction() != null) {
                SqlNode condition = whenCtx.condition != null ? visit(whenCtx.condition) : null;

                // Pass condition into mergeMatchedAction method
                SqlNode action = visitMergeMatchedAction(whenCtx.mergeMatchedAction(), condition);

                if (!(action instanceof SqlUpdate || action instanceof SqlDelete)) {
                    throw new UnsupportedOperationException("WHEN MATCHED action must be UPDATE or DELETE.");
                }

                matchedClauses.add(new SqlMerge.MatchedClause(condition, action));
            }

            // Handle WHEN NOT MATCHED BY TARGET
            else if (whenCtx.mergeNotMatchedByTargetAction() != null) {
                insertCall = (SqlInsert) visit(whenCtx.mergeNotMatchedByTargetAction());
                if (whenCtx.condition != null) {
                    insertCondition = visit(whenCtx.condition);
                }
            }

        }

        this.currentMergeTargetTable = null;
    ...
    public SqlNode visitMergeMatchedAction(BigQuerySqlParser.MergeMatchedActionContext ctx, SqlNode condition) {
        if (ctx.DELETE() != null) {
            return new SqlDelete(
                    parserPos(ctx),
                    this.currentMergeTargetTable,
                    condition,
                    null,      // alias is not needed as it's already wrapped in AS
                    null       // hints
            );
        }

        SqlNodeList targetColumnList = new SqlNodeList(parserPos(ctx));
        SqlNodeList sourceExpressionList = new SqlNodeList(parserPos(ctx));

        if (ctx.updateSetItem() != null) {
            for (BigQuerySqlParser.UpdateSetItemContext setItemCtx : ctx.updateSetItem()) {
                if (setItemCtx.primaryExpression() != null) {
                    SqlNode column = visit(setItemCtx.primaryExpression());
                    SqlNode expr = visit(setItemCtx.expression(0));

                    if (column != null && expr != null) {
                        targetColumnList.add(column);
                        sourceExpressionList.add(expr);
                    }
                } else if (!setItemCtx.columnReference().isEmpty()) {
                    for (int i = 0; i < setItemCtx.columnReference().size(); i++) {
                        SqlNode column = visit(setItemCtx.columnReference(i));
                        if (i < setItemCtx.expression().size()) {
                            SqlNode expr = visit(setItemCtx.expression(i));
                            if (column != null && expr != null) {
                                targetColumnList.add(column);
                                sourceExpressionList.add(expr);
                            }
    ...
    @Override
    public SqlNode visitMergeNotMatchedByTargetAction(BigQuerySqlParser.MergeNotMatchedByTargetActionContext ctx) {
        SqlNodeList columnList = new SqlNodeList(parserPos(ctx));
        SqlNodeList valueList = new SqlNodeList(parserPos(ctx));

        for (int i = 0; i < ctx.columnName().size(); i++) {
            SqlColumnIdentifier column = createColumnIdentifier(ctx.columnName(i).identifier());
            columnList.add(column);
        }

        for (int i = 0; i < ctx.expression().size(); i++) {
            valueList.add(visit(ctx.expression(i)));
        }
    ...
    @Override
    public SqlNode visitMergeNotMatchedBySourceAction(BigQuerySqlParser.MergeNotMatchedBySourceActionContext ctx) {
        return new SqlDelete(
                parserPos(ctx),
                this.currentMergeTargetTable,
                null,
    ...
    @Override
    public SqlNode visitInsertStatement(BigQuerySqlParser.InsertStatementContext ctx) {
        // Get target table identifier
        SqlIdentifier targetTable = (SqlIdentifier) visit(ctx.tableIdentifier());

        // Handle optional alias (usually not needed for INSERT, but supported in BigQuery)
        SqlIdentifier alias = null;
        if (ctx.alias != null) {
            alias = createIdentifier(ctx.alias);
        }

        // Create a column list if specified
        SqlNodeList columnList = null;
        if (ctx.columnReference() != null && !ctx.columnReference().isEmpty()) {
            List<SqlNode> columns = ctx.columnReference().stream()
                    .map(this::visit)
                    .collect(Collectors.toList());
            columnList = new SqlNodeList(columns, parserPos(ctx));
        }
        ...
        if (ctx.selectStatement() != null) {
            // INSERT from a query
            source = visit(ctx.selectStatement());
        } else if (ctx.valueRow() != null && !ctx.valueRow().isEmpty()) {
            // INSERT from VALUES
            List<SqlNode> rows = ctx.valueRow().stream()
                    .map(this::visit)
                    .collect(Collectors.toList());
            source = SqlStdOperatorTable.VALUES.createCall(parserPos(ctx), rows);
            ...
            SqlNode row = SqlStdOperatorTable.ROW.createCall(parserPos(ctx), expressions);
            source = SqlStdOperatorTable.VALUES.createCall(parserPos(ctx), row);
        } else {
            throw new UnsupportedOperationException("Unsupported INSERT source: " + ctx.getText());
        }

        // Create the target with optional alias
        SqlNode targetWithAlias = targetTable;
        if (alias != null) {
            targetWithAlias = new SqlBasicCall(
                    SqlStdOperatorTable.AS,
                    List.of(targetTable, alias),
                    parserPos(ctx));
        }

        // Create INSERT statement
        SqlNode insertStatement = new SqlInsert(
                parserPos(ctx),
                SqlNodeList.EMPTY,  // No keywords
                targetWithAlias,    // Target table with optional alias
                source,             // Source (query/values)
                columnList          // Optional column list
        );

        // Wrap with WITH clause if present
        if (ctx.withClause() != null) {
            SqlNodeList withList = (SqlNodeList) visit(ctx.withClause());
            insertStatement = new SqlWith(parserPos(ctx), withList, insertStatement);
        }
    ...
    @Override
    public SqlNode visitValueRow(BigQuerySqlParser.ValueRowContext ctx) {
        // Extract all expressions in the row
        List<SqlNode> values = new ArrayList<>();
        if (ctx.expression() != null && !ctx.expression().isEmpty()) {
            values = ctx.expression().stream()
                    .map(this::visit)
                    .collect(Collectors.toList());
        }

        // Create a ROW constructor call with the expressions
...
//    }
    ...
    public SqlNode visitTableIdentifier(BigQuerySqlParser.TableIdentifierContext ctx) {
        List<String> names = ctx.hyphenatedIdentifier().stream()
                .map(it -> ((SqlIdentifier) visit(it)).getSimple())
                .toList();
        return new SqlTableIdentifier(names, parserPos(ctx));
    }

    @Override
    public SqlNode visitHyphenatedIdentifier(BigQuerySqlParser.HyphenatedIdentifierContext ctx) {
        // Check if we have IDENTIFIER tokens
        if (ctx.IDENTIFIER() != null && !ctx.IDENTIFIER().isEmpty()) {
            StringBuilder identifier = new StringBuilder();
            identifier.append(ctx.IDENTIFIER(0).getText());

            // Append all subsequent parts with hyphens
            for (int i = 1; i < ctx.IDENTIFIER().size(); i++) {
                identifier.append('-').append(ctx.IDENTIFIER(i).getText());
            }

            return new SqlIdentifier(identifier.toString(), parserPos(ctx));
        } else if (ctx.QUOTED_IDENTIFIER() != null) {
            // Handle quoted identifiers
            String text = ctx.QUOTED_IDENTIFIER().getText();
            // Remove the surrounding quotes
            text = text.substring(1, text.length() - 1).replace("\"\"", "\"");
            return new SqlIdentifier(text, parserPos(ctx));
        ...
        throw new UnsupportedOperationException(
                "Invalid hyphenated identifier format: " + ctx.getText());
    }
...
//  }
    ...
    /**
     * Creates a SqlColumnIdentifier from an identifier context.
     * This is used specifically for column references in INSERT statements and similar contexts.
     */
    private SqlColumnIdentifier createColumnIdentifier(BigQuerySqlParser.IdentifierContext ctx) {
        if (ctx == null) {
            return null;
        }
        ...
        return new SqlColumnIdentifier(identifierText, parserPos(ctx));
    }
    ...
    public SqlNode visitEqualityExpression(BigQuerySqlParser.EqualityExpressionContext ctx) {
        ...
        for (int i = 0; i < ctx.comparisonExpression().size() - 1; i++) {
            ...
            SqlOperator operator;
            if (ctx.EQ() != null && !ctx.EQ().isEmpty()) {
                operator = SqlStdOperatorTable.EQUALS;
            } else { // NE (<>)
                operator = NotEqualsOverrideOperator.INSTANCE;
            }
...
//        }
    ...
    private SqlNode createAggFunction(SqlAggFunction aggFunction, boolean distinct, SqlNode... operands) {
        if (distinct && operands.length > 0) {
            return aggFunction.createCall(
                    SqlSelectKeyword.DISTINCT.symbol(SqlParserPos.ZERO),
                    SqlParserPos.ZERO,
                    operands);
        } else {
...
//    }
        return new SqlColumnIdentifier(names, parserPos(ctx));
    }
